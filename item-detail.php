...
<head>
    ...
    <title><?php echo htmlspecialchars($item['name']); ?> - 奇楠沉香交易拍賣平台</title>
    <style>
        body {
        ...
        <div class="item-detail">
            <div class="images-section">
                <?php if ($images_result->num_rows > 0): ?>
                    ...
                    <img src="<?php echo $first_image['file_path']; ?>" alt="<?php echo htmlspecialchars($item['name']); ?>" class="main-image" id="mainImage">
            ...
            <div class="info-section">
                ...
                <table class="info-table">
                    ...
                    <tr>
                        ...
                        <td><?php 
                            if ($item['seller_name']) {
                                $first_char = mb_substr($item['seller_name'], 0, 1, 'UTF-8');
                                $rest_chars = str_repeat('*', mb_strlen($item['seller_name'], 'UTF-8') - 1);
                                echo htmlspecialchars($first_char . $rest_chars);
                            } else {
                                echo '未知';
                            }
                        ?></td>
                ...
                <?php if ($item['remarks']): ?>
                    <div>
                        <strong>其他備註：</strong>
                        <p><?php echo nl2br(htmlspecialchars($item['remarks'])); ?></p>
                    </div>
                <?php endif; ?>
                    ...
                    <tbody>
                        <?php while ($bid = $bids_result->fetch_assoc()): ?>
                            <tr>
                                <td><?php 
                                    if ($bid['bidder_name']) {
                                        $first_char = mb_substr($bid['bidder_name'], 0, 1, 'UTF-8');
                                        $rest_chars = str_repeat('*', mb_strlen($bid['bidder_name'], 'UTF-8') - 1);
                                        echo htmlspecialchars($first_char . $rest_chars);
                                    } else {
                                        echo '匿名用戶';
                                    }
